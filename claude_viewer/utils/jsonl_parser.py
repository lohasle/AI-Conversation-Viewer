...
class JSONLParser:
    def __init__(self, projects_path: str = None, parser_type: str = "claude"):
        ...
        if parser_type == "claude":
            self.projects_path = projects_path or os.path.expanduser(
                "~/.claude/projects"
            )
            ...
            self.projects_path = projects_path or os.environ.get(
                "CURSOR_WORKSPACE_STORAGE_PATH", default_path
            )
            ...
            self.projects_path = projects_path or os.environ.get(
                "TRAE_WORKSPACE_STORAGE_PATH", default_path
            )
            ...
            self.projects_path = projects_path or os.environ.get(
                "KIRO_WORKSPACE_STORAGE_PATH", default_path
            )
            # Sessions are stored in globalStorage workspace-sessions
            self.kiro_sessions_path = os.path.join(
                os.path.expanduser("~"),
                "Library",
                "Application Support",
                "Kiro",
        ...
        for project_dir in os.listdir(self.projects_path):
            ...
            if os.path.isdir(project_path):
                if self.parser_type == "claude":
                    session_files = [
                        f for f in os.listdir(project_path) if f.endswith(".jsonl")
                    ]
                elif self.parser_type == "qwen":
                    chats_path = os.path.join(project_path, "chats")
                    if os.path.exists(chats_path):
                        session_files = [
                            f for f in os.listdir(chats_path) if f.endswith(".json")
                        ]
                    else:
                        session_files = []
                        ...
                        try:
                            ...
                            with open(workspace_json, "r", encoding="utf-8") as f:
                                ...
                                sessions_path = os.path.join(
                                    self.kiro_sessions_path, sessions_dir_name
                                )

                                # Check sessions.json
                                sessions_json_file = os.path.join(
                                    sessions_path, "sessions.json"
                                )
                                if os.path.exists(sessions_json_file):
                                    with open(
                                        sessions_json_file, "r", encoding="utf-8"
                                    ) as sf:
                ...
                if self.parser_type in ["cursor", "trae"]:
                    ...
                    display_name = (
                        self._format_cursor_display_name(project_dir) or display_name
                    )
                elif self.parser_type == "kiro":
                    # For Kiro, decode the base64 directory name
                    display_name = (
                        self._format_kiro_display_name(project_dir) or display_name
                    )
                projects.append(
                    {
                        "name": project_dir,
                            ...
                            with open(sessions_json_file, "r", encoding="utf-8") as sf:
                                ...
                                for session_info in sessions_data:
                                    ...
                                    session_file = os.path.join(
                                        sessions_path, f"{session_id}.json"
                                    )
    ...
    def get_conversation(
        ...
        if self.parser_type == "claude":
            session_path = os.path.join(
                self.projects_path, project_name, f"{session_id}.jsonl"
            )
        elif self.parser_type == "qwen":
            project_path = os.path.join(self.projects_path, project_name)
                ...
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        if self._should_include_message(
                            parsed_message, search, message_type
                        ):
    ...
    def _parse_cursor_message(self, data: Dict, line_num: int) -> Dict:
        ...
        if role is None:
            if data.get("from"):
                role = (
                    "user" if str(data.get("from")).lower() == "user" else "assistant"
                )
        ...
        content = (
            ...
            or data.get("prompt")
            or data.get("message")
            or data.get("inputText")
            or data.get("outputText")
            or data.get("value")
            or data.get("body")
            or data.get("textContent")
            or ""
        )
        if isinstance(content, list):
        ...
        for item in content_list:
            if isinstance(item, dict):
                if item.get("type") == "text":
                    ...
                    if (
                        tool_name == "Edit"
                        and tool_params.get("old_string")
                        and tool_params.get("new_string")
                    ):
                    ...
                    if (
                        tool_use_result
                        and tool_use_result.get("oldString")
                        and tool_use_result.get("newString")
                    ):
    ...
    def _should_include_message(
        self, message: Dict, search: Optional[str], message_type: Optional[str]
    ) -> bool:
        ...
        if self.parser_type == "claude":
            ...
            if os.path.exists(chats_path):
                ...
                if session_files:
                    ...
                    session_files.sort(
                        key=lambda x: os.path.getmtime(os.path.join(chats_path, x)),
                        reverse=True,
                    )
                    for session_file in session_files:
                        try:
                            with open(
                                os.path.join(chats_path, session_file),
                                "r",
                                encoding="utf-8",
                            ) as f:
                                session_data = json.load(f)
                                ...
                                project_hash_in_session = session_data.get(
                                    "projectHash", ""
                                )
            ...
            def _extract_path(data_obj):
                ...
                cand = (
                    ...
                    or data_obj.get("workspace")
                    or data_obj.get("workspacePath")
                ...
                if (
                    not cand
                    and isinstance(data_obj.get("folders"), list)
                    and data_obj["folders"]
                ):
                ...
                if not cand and conf:
                    try:
                        ...
                        if (
                            isinstance(conf_obj, dict)
                            and isinstance(conf_obj.get("folders"), list)
                            and conf_obj["folders"]
                        ):
                        ...
                        try:
                            ...
                            def collect_name_from_uri(uri: str):
                                try:
                                    ...
                                    found = None
                                    for _ in range(6):
                                        if os.path.isdir(probe) and os.path.exists(
                                            os.path.join(probe, ".git")
                                        ):
                                            found = os.path.basename(probe)
                                            break
                                        new_probe = os.path.dirname(
                                            probe.rstrip(os.sep)
                                        )
                                        if new_probe == probe:
                                            break
    ...
    def _generate_diff_html(
        ...
        if not diff:
            return (
                f"<div class='diff-no-changes'>No changes detected in {file_path}</div>"
            )
        ...
        html_lines.append(
            f'<div class="diff-header">📝 <strong>File:</strong> {file_path}</div>'
        )
        ...
        for line in diff:
            if line.startswith("@@"):
                ...
                html_lines.append(
                    f'<span class="diff-line-number">{line_num_old}</span>'
                )
                html_lines.append(f'<span class="diff-marker">-</span>')
                html_lines.append(
                    f'<span class="diff-content">{self._escape_html(content)}</span>'
                )
                ...
                html_lines.append(
                    f'<span class="diff-line-number">{line_num_new}</span>'
                )
                html_lines.append(f'<span class="diff-marker">+</span>')
                html_lines.append(
                    f'<span class="diff-content">{self._escape_html(content)}</span>'
    ...
    def _cursor_count_prompts(self, db_path: str) -> int:
        ...
        def _normalize_item(item):
            ...
            if isinstance(item, dict):
                payload = (
                    _parse_json_maybe(item.get("value"))
                ...
                if isinstance(payload, dict):
                    ...
                    if role is None:
                        if payload.get("from"):
                            role = (
                                "user"
                                if str(payload.get("from")).lower() == "user"
                                else "assistant"
                            )
            ...
            for item in items:
                ...
                if isinstance(norm, list):
                    ...
                    if (
                        ...
                        and isinstance(norm.get("content"), str)
                        and norm.get("content").strip()
                    ):
                        results.append(norm)
            return results
        except Exception:
    ...
    def _find_state_key(self, cur) -> Optional[str]:
        ...
        for pat in patterns:
            try:
                cur.execute(
                    "SELECT key FROM ItemTable WHERE key LIKE ? LIMIT 100", (pat,)
                )
                            ...
                            def visit(x):
                                ...
                                if isinstance(x, list):
                                    if (
                                        ...
                                        and any(k in x[0] for k in text_keys)
                                    ):
                                        l = len(x)
                                        if l > best_len:
                                            best = x
        ...
        try:
            cur.execute(
                "SELECT key, value FROM ItemTable ORDER BY LENGTH(value) DESC LIMIT 50"
            )
            rows = cur.fetchall()
            for key, val in rows:
