...
Examples:
  ...
  claude-viewer --host 0.0.0.0 --port 3000      # Accessible from other machines
    ...
    parser.add_argument(
        "--version", action="version", version=f"claude-viewer {__version__}"
    )
    ...
    if not validate_projects_path(args.projects_path):
        print(
            "⚠️ Proceeding without valid Claude projects; other views may still work (Cursor/Trae/Kiro/Qwen)."
        )
    # Set environment variable for the app to use regardless
    os.environ["CLAUDE_PROJECTS_PATH"] = str(
        Path(args.projects_path).expanduser().resolve()
    )
